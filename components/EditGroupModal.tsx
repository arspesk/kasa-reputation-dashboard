...
interface EditGroupModalProps {
  ...
  onGroupUpdated: (group: GroupWithDetails) => void;
}
...
export default function EditGroupModal({
  ...
  const [initialHotelIds, setInitialHotelIds] = useState<Set<string>>(new Set());
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const hotelIdsToAdd = Array.from(selectedHotelIds).filter(
        (id) => !initialHotelIds.has(id)
      );
      const hotelIdsToRemove = Array.from(initialHotelIds).filter(
        (id) => !selectedHotelIds.has(id)
      );

      // Remove hotels
      if (hotelIdsToRemove.length > 0) {
        const { error: removeError } = await supabase
          .from("hotel_group_members")
          .delete()
          .eq("group_id", group.id)
          .in("hotel_id", hotelIdsToRemove);

        if (removeError) throw removeError;
      }

      // Add hotels
      if (hotelIdsToAdd.length > 0) {
        const { error: addError } = await supabase
          .from("hotel_group_members")
          .insert(
            hotelIdsToAdd.map((hotel_id) => ({
              group_id: group.id,
              hotel_id,
            }))
          );
      ...
      onGroupUpdated({
        ...group,
        name: trimmedName,
        member_count: selectedHotelIds.size,
      });
          ...
          <div className="p-6 overflow-y-auto flex-1">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-800 mb-2">
                Hotels in this Group ({selectedHotelIds.size})
              </label>
